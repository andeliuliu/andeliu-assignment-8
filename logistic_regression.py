import matplotlib
matplotlib.use('Agg')  # Use non-GUI backend for headless plotting
import numpy as np
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        # Generate clusters with a given shift distance
        ...
        logistic_loss = -np.mean(y * np.log(prob_predictions[:, 1]) + (1 - y) * np.log(1 - prob_predictions[:, 1]))
